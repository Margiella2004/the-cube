...
const QUALITY = clamp(Number(qs.get("q") || 2), 0.5, 2);
const DOF_ENABLED = qs.get("dof") === "1";
const CINEMATIC = qs.get("cinematic") !== "0";
const BLOOM = clamp(Number(qs.get("bloom") || 0.32), 0, 1.5);
const GRAIN = clamp(Number(qs.get("grain") || 0.12), 0, 1);
const VIGNETTE = clamp(Number(qs.get("vignette") || 1.15), 0, 2);
const CHROMA = clamp(Number(qs.get("chroma") || 0.00045), 0, 0.01);
...
// Portfolio/C4D-like studio lighting.
...
// Postprocessing.
const renderTarget = new THREE.WebGLRenderTarget(1, 1, { depthBuffer: true, stencilBuffer: false });
...
const bloomPass = new UnrealBloomPass(new THREE.Vector2(1, 1), BLOOM, 0.75, 0.92);
...
const filmPass = new FilmPass(GRAIN, 0.0, 0, false);
composer.addPass(filmPass);

// Simple inlined sharpen pass (avoids relying on UnsharpMaskPass, which isn't present in all Three versions).
const SharpenShader = {
  uniforms: {
    tDiffuse: { value: null },
...
// Ground shadow catcher (contact-shadow-ish).
const shadowPlane = new THREE.Mesh(new THREE.PlaneGeometry(200, 200), new THREE.ShadowMaterial({ opacity: 0.4 }));
...
function detectCommonCubeMaterials() {
  ...
  console.log(
    "[GLB/Vite] Materials used on ALL cubes:",
